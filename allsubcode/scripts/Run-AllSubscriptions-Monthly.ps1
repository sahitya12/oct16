...
variables:
  # Using your variable group for login (as per screenshots)
...
- task: PowerShell@2
  ...
  inputs:
    ...
    script: |
      ...
      $scriptPath = "$(Build.SourcesDirectory)\sanitychecks\scripts\Run-AllSubscriptions-Monthly.ps1"
      if (-not (Test-Path -LiteralPath $scriptPath)) { throw "Missing orchestrator: $scriptPath" }
