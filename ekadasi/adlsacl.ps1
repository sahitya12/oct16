...
# ----------------------------------------------------------------------
# PRE-FLIGHT: prevent agent/pipeline cached Az context from forcing 0000.. subscription
# (Do this BEFORE Connect-ScAz)
# ----------------------------------------------------------------------
try {
    Import-Module Az.Accounts -ErrorAction Stop

    # Stop Az from reusing cached contexts from agent profile
    Disable-AzContextAutosave -Scope Process | Out-Null

    # Clear process context (safe for the current script run)
    ...
    Write-Host "DEBUG: Pre-flight context cleanup skipped: $($_.Exception.Message)"
}

# ----------------------------------------------------------------------
# Connect to Azure (via Common.psm1)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# POST-FLIGHT: ensure we have a valid initial context (avoid 0000..)
# Your loop later uses Set-ScContext per subscription anyway.
# ----------------------------------------------------------------------
try {
    $subsVisible = @(Get-AzSubscription -ErrorAction Stop)
    Write-Host ("DEBUG: Accessible subscriptions after login = {0}" -f $subsVisible.Count)

    if ($subsVisible.Count -gt 0) {
        $first = $subsVisible | Select-Object -First 1
        Set-AzContext -SubscriptionId $first.Id -ErrorAction Stop | Out-Null
        Write-Host "DEBUG: Initial context set to $($first.Name) / $($first.Id)"
    }
    else {
