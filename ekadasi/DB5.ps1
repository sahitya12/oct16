...
# ✅ FIX COLUMN ORDER: Note column will always be LAST (even if file has only dummy row)
#    - Ensure-AtLeastOneRow uses [ordered] hashtables
#    - Export uses Select-Object with explicit column order
# ------------------------------------------------------------------------------------
...
function Get-DbxPatFromSpn {
  ...
  if (-not $aadTok) {
    return @{ Ok=$false; AadToken=$null; Pat=$null; TokenId=$null; Reason="Failed to get AAD token (client_credentials)"; Last=$script:LastDbxError }
  }
...
# -------- Ensure not-empty outputs (ORDERED so headers stable) --------
Ensure-AtLeastOneRow ([ref]$wsRows)   ([ordered]@{ SubscriptionName=''; SubscriptionId=''; ResourceGroup=''; WorkspaceName=''; Location=''; WorkspaceUrl=''; WorkspaceResourceId=''; Note='' })
Ensure-AtLeastOneRow ([ref]$whRows)   ([ordered]@{ SubscriptionName=''; ResourceGroup=''; WorkspaceName=''; WarehouseId=''; WarehouseName=''; State=''; ClusterSize=''; Note='' })
Ensure-AtLeastOneRow ([ref]$whPerms)  ([ordered]@{ SubscriptionName=''; WorkspaceName=''; WarehouseName=''; WarehouseId=''; PrincipalType=''; PrincipalName=''; PermissionLevel=''; Inherited=''; Note='' })
Ensure-AtLeastOneRow ([ref]$catRows)  ([ordered]@{ SubscriptionName=''; ResourceGroup=''; WorkspaceName=''; CatalogName=''; Owner=''; Comment=''; Note='' })
Ensure-AtLeastOneRow ([ref]$catPerms) ([ordered]@{ SubscriptionName=''; WorkspaceName=''; CatalogName=''; PrincipalName=''; Privileges=''; Note='' })
Ensure-AtLeastOneRow ([ref]$extRows)  ([ordered]@{ SubscriptionName=''; ResourceGroup=''; WorkspaceName=''; ExternalLocation=''; Url=''; Owner=''; Comment=''; Note='' })
Ensure-AtLeastOneRow ([ref]$extPerms) ([ordered]@{ SubscriptionName=''; WorkspaceName=''; ExternalLocation=''; PrincipalName=''; Privileges=''; Note='' })
Ensure-AtLeastOneRow ([ref]$authRows) ([ordered]@{ SubscriptionName=''; WorkspaceName=''; WorkspaceUrl=''; AuthMode=''; AuthSpnUsed=''; AuthOk=''; Note='' })
...
# -------- EXPORT with explicit column order (guaranteed "Note" last) --------
$wsRows  | Select-Object SubscriptionName,SubscriptionId,ResourceGroup,WorkspaceName,Location,WorkspaceUrl,WorkspaceResourceId,Note `
        | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvWs

$whRows  | Select-Object SubscriptionName,ResourceGroup,WorkspaceName,WarehouseId,WarehouseName,State,ClusterSize,Note `
        | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvWh

$whPerms | Select-Object SubscriptionName,WorkspaceName,WarehouseName,WarehouseId,PrincipalType,PrincipalName,PermissionLevel,Inherited,Note `
        | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvWhP

$catRows | Select-Object SubscriptionName,ResourceGroup,WorkspaceName,CatalogName,Owner,Comment,Note `
        | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvCat

$catPerms | Select-Object SubscriptionName,WorkspaceName,CatalogName,PrincipalName,Privileges,Note `
         | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvCatP

$extRows | Select-Object SubscriptionName,ResourceGroup,WorkspaceName,ExternalLocation,Url,Owner,Comment,Note `
        | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvExt

$extPerms | Select-Object SubscriptionName,WorkspaceName,ExternalLocation,PrincipalName,Privileges,Note `
         | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvExtP

$authRows | Select-Object SubscriptionName,WorkspaceName,WorkspaceUrl,AuthMode,AuthSpnUsed,AuthOk,Note `
         | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $csvAuth
